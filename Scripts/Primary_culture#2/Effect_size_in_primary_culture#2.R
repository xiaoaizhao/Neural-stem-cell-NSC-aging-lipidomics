...
#### Effect size on individual lipid species - only include lipids with quantitative standards ############################################
##calculate effect size on primary culture #2 data####
rm(list = ls())
source("./Scripts/Function_scripts/Effect_size_functions.R")
load(file = "./Output_Data/Exp2_Norm_Impt_backtoraw_conc612_lipids.Rdata") ##load pre-processed matrix with lipid concentration
...
##effect size on age difference at individual lipid level within each KO condition
KO.list <- unique(conc.lipid.df$KO)
...
Exp2.CONC.lpd.es.g.allKO <- bind_rows(EfSize.df)
save(Exp2.CONC.lpd.es.g.allKO, file = "./Output_Data/Ef_Size_CONC.Lipid_Exp2_all_KO.Rdata")
...
#### Effect size on double bond composition - lipids with quantitative standards only####
load("./Output_Data/Exp2_CONC.DB_PCT_all_samples.Rdata")
Exp2_CONC.DB <- Exp2_CONC.DB %>%
  mutate(., Cla_DB = paste0(Class, DB_num)) %>%
  mutate(., KO = substr(Sample, nchar(Sample), nchar(Sample)))

##effect size on age difference at double bond composition level within each KO condition
KO.list <- unique(Exp2_CONC.DB$KO)
...
Exp2.CONC.DB.es.g.allKO <- bind_rows(EfSize.df.DB)
save(Exp2.CONC.DB.es.g.allKO, file = "./Output_Data/Ef_Size_CONC.DB_PCT_Exp2_all_KO.Rdata")
