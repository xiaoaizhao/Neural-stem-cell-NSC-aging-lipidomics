...
## ====calculate effect size on qNSC from In vitro, side chain analysis====####
load("./Output_Data/Invitro.SC.analysis.Rdata")

SC.invitro <- Invitro.SumSC %>% 
  mutate(Cla_SC = paste0(Class, "(", Sep.SC, ")")) %>% 
  mutate(Age = ifelse(grepl("^O", Samples), "Old", "Young")) 
  
Invitro.Qui.SC.es.g <- es.g.func(SC.invitro, Cla_SC, Age, SumSC, Samples)
save(Invitro.Qui.SC.es.g, file = "./Output_Data/SC.abundance.Invitro_Qui_Age_ES.Rdata")
