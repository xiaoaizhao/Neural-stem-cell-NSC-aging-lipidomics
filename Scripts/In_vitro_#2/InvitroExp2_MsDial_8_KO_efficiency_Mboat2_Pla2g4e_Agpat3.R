...
load(file = "./Output_Data/KO_LUT_paper.order2024-02-08.Rdata")
load("./Output_Data/Exp2_278_conc_lipid.MSD.format.Rdata")

Exp2.raw <- E2MsD.frmt

KO.df <- MsD.lpd.rmv.abc(Exp2.raw) %>% 
...
ctrl_mboat2_clr <- KO.LUT.paperorder$Color[KO.LUT.paperorder$KO %in% ls]
...
KO.name <- "Pla2g4e"
ls <- c("Control", KO.name)
ctrl_pla_clr <- KO.LUT.paperorder$Color[KO.LUT.paperorder$KO %in% ls]
...
LPC_16_0 <- KO.df %>%
  ...
  mutate(., KO = substr(Sample, 3, 4)) %>%
  group_by(., Sample) %>%
  summarise(., LPC_16_0_all = sum(Concentration))

PC_16_0 <- KO.df %>%
  filter(., Class == "PC" & grepl("16:0", SideChain)) %>%
  mutate(., KO = substr(Sample, 3, 4)) %>%
  group_by(., Sample) %>%
  summarise(., PC_16_0_all = sum(Concentration))
