...
## Steps
#1. lipid with unique ID (852), 270 left after ion clean.
#2. lipid with multiple ID (2284) - general rules: keep lipid that has the highest overall grade
  #1. lipids with one identification that has the highest grade (201, m.mult.1)
  #2. lipids with multiple identification with identical highest grade (2083), then take lipids with the highest median intensity
    #1. 2041 unique lipid can be identified based on the highest median intensity, m.mlt.grade
    #2. 42 lipids have identical value of highest median intensity, randomly pick one identification(first one in the list), m.mlt.grade.2c
# final cleaned positive ion = 1122
rm(list=ls())
setwd(rstudioapi::getActiveProject())
library(openxlsx)
library(tidyverse)

load("./Output_Data/Matched_Batch2_pos_by_LipidMol.Rdata")
Match_pos.s <- Match2_pos.mol %>%
  relocate(LipidIon, .after = Compound) %>%
  filter(!LipidIon == "") #3136
...
uni.ID <- Match_pos.s %>% 
  filter(., !grepl("\\|", LipidIon)) #852 lipids with only one annotation
...
m.mult.1 <- multi.id %>% 
  filter(length(unlist(ID.toKeep)) == 1) #201
...
m.mult.2 <- multi.id %>% 
  filter(!length(unlist(ID.toKeep)) == 1) #2083
...
m.mlt.grade <- m.mult.2 %>% #2083
  ...
  relocate(c("ID.toKeepI","MedInt.toKeepI", "MedInt.toKeep"),
           .after =  "ID.toKeep")  %>% 
  filter(length(unlist(ID.toKeepI)) == 1) #2041
...
m.mlt.grade2 <-  m.mult.2 %>% 
  ...
  relocate(c("ID.toKeepI","MedInt.toKeepI", "MedInt.toKeep"),
           .after =  "ID.toKeep")  %>% 
  filter(!length(unlist(ID.toKeepI)) == 1) #42
...
Mlt.clean <- Mult.to.clean %>% #2284
  ...
  relocate(c("Ion","ion_match", "Grade_LS"), .after =  "LipidIon") %>% 
  filter(., ion_match == "T")  %>%
  arrange(., Ion) ##852 after ion cleanup.

M2PM.pos.clean <- bind_rows(uni.ID.c, Mlt.clean) #1122

save(M2PM.pos.clean, file = "./Output_Data/M2PM_pos_ion_clean.Rdata")
...
## all checks out!!
i = 239
ins.df <- Match_pos.s[i, c("Compound", "LipidIon", "Grade_LS", "Med_LS_Int")]
ins.df

check.df <- M2PM.pos.clean[M2PM.pos.clean$Compound == ins.df$Compound,1:4]
check.df
