...
## Steps
#1. lipid with unique ID (1072), 361 left after ion clean.
#2. lipid with multiple ID (1653) - general rules: keep lipid that has the highest overall grade
  #1. lipids with one identification that has the highest grade (214, m.mult.1)
  #2. lipids with multiple identification with identical highest grade (1439), then take lipids with the highest median intensity
      #1. 1377 unique lipid can be identified based on the highest median intensity, m.mlt.grade
      #2. 62 lipids have identical value of highest median intensity, randomly pick one identification(first one in the list), m.mlt.grade.2c

#1095 peaks after ion clean (10ppm), up from 1084 (5ppm)
rm(list=ls())
setwd(rstudioapi::getActiveProject())
library(openxlsx)
library(tidyverse)

load("./Output_Data/Matched_Batch3_pos_by_LipidMol.Rdata")
Match_pos.s <- Match3_pos.mol %>%
...
uni.ID <- Match_pos.s %>% 
  filter(., !grepl("\\|", LipidIon)) #1072 lipids with only one annotation
...
uni.ID.c <-uni.ID %>%
  ...
  filter(., ion_match == "T")  %>%
  arrange(., Ion) ##361 after ion cleanup.
...
multi.id <- Match_pos.s %>% 
  filter(., grepl("\\|", LipidIon)) %>% #1653
...
m.mult.1 <- multi.id %>% 
  filter(length(unlist(ID.toKeep)) == 1) #214
m.mult.1$ID.toKeep <- as.character(m.mult.1$ID.toKeep)
m.mult.1$MZ.toKeep <- as.character(m.mult.1$MZ.toKeep)
m.mult.1$RT.toKeep <- as.character(m.mult.1$RT.toKeep)
...
m.mult.2 <- multi.id %>% 
  filter(!length(unlist(ID.toKeep)) == 1) #1439
...
m.mlt.grade <- m.mult.2 %>% #1439
  ...
  relocate(c("ID.toKeepI","MedInt.toKeepI", "MedInt.toKeep"),
           .after =  "ID.toKeep")  %>% 
  filter(length(unlist(ID.toKeepI)) == 1) #1377
...
m.mlt.grade2 <-  m.mult.2 %>% 
  ...
  relocate(c("ID.toKeepI","MedInt.toKeepI", "MedInt.toKeep"),
           .after =  "ID.toKeep")  %>% 
  filter(!length(unlist(ID.toKeepI)) == 1) #62
...
Mlt.clean <- Mult.to.clean %>% 
  ...
  relocate(c("Ion","ion_match", "Grade_LS"), .after =  "LipidIon") %>% 
  filter(., ion_match == "T")  %>%
  arrange(., Ion) ##734 after ion cleanup.

Exp3.pos.clean <- bind_rows(uni.ID.c, Mlt.clean) #1095

save(Exp3.pos.clean, file = "./Output_Data/Exp3_pos_ion_clean.Rdata")
