...
## Steps
#1. lipid with unique ID (646), 130 left after ion clean.
#2. lipid with multiple ID (750) - general rules: keep lipid that has the highest overall grade
  #1. lipids with one identification that has the highest grade (259, m.mult.1)
  #2. lipids with multiple identification with identical highest grade (491), then take lipids with the highest median intensity
    #1. 473 unique lipid can be identified based on the highest median intensity, m.mlt.grade
    #2. 18 lipids have identical value of highest median intensity, randomly pick one identification(first one in the list), m.mlt.grade.2c
# 305 peaks after ion clean (10 ppm), up from 287 (5ppm).
rm(list=ls())
setwd(rstudioapi::getActiveProject())
library(openxlsx)
library(tidyverse)

load("./Output_Data/Matched_Batch3_neg_by_LipidMol.Rdata")
Match_neg.s <- Match3_neg.mol %>%
...
uni.ID <- Match_neg.s %>% 
  filter(., !grepl("\\|", LipidIon)) #646 lipids with only one annotation
...
uni.ID.c <-uni.ID %>%
  ...
  filter(., ion_match == "T")  %>%
  arrange(., Ion) ##130 after ion cleanup.
...
m.mult.1 <- multi.id %>% 
  filter(length(unlist(ID.toKeep)) == 1) #259
...
m.mult.2 <- multi.id %>% 
  filter(!length(unlist(ID.toKeep)) == 1) #491
...
m.mlt.grade <- m.mult.2 %>% #491
  ...
  relocate(c("ID.toKeepI","MedInt.toKeepI", "MedInt.toKeep"),
           .after =  "ID.toKeep")  %>% 
  filter(length(unlist(ID.toKeepI)) == 1) #473
...
m.mlt.grade2 <-  m.mult.2 %>% 
  ...
  relocate(c("ID.toKeepI","MedInt.toKeepI", "MedInt.toKeep"),
           .after =  "ID.toKeep")  %>% 
  filter(!length(unlist(ID.toKeepI)) == 1) #18
...
Mlt.clean <- Mult.to.clean %>% 
  ...
  relocate(c("Ion","ion_match", "Grade_LS"), .after =  "LipidIon") %>% 
  filter(., ion_match == "T")  %>%
  arrange(., Ion) ##175 after ion cleanup.

Exp3.neg.clean <- bind_rows(uni.ID.c, Mlt.clean) #305

save(Exp3.neg.clean, file = "./Output_Data/Exp3_neg_ion_clean.Rdata")
