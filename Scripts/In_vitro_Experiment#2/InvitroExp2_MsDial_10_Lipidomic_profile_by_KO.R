### In vitro Experiment #2 - Analysis on overlapping sets of lipids that are increased or decreased in each KO compared to control 
...
### old samples only, use a different effect size calculation so that 
KO.l.old <- KO.l %>% 
...
for (ko.t in ko.target) {
  es.ko[[ko.t]] <- KO.l.old %>% 
    filter(KO == "N" | KO == ko.t)
  df.for.es <- bind_rows(es.ko[[ko.t]])
  es.ko1[[ko.t]] <- es.g.func.KO.to.ctrl(df.for.es, LipidIon, KO, Concentration, Sample) %>% 
...
save(all.ko.es.old, file = "./Output_Data/Exp2_lpd.es.by.KO.in.old.Rdata")
