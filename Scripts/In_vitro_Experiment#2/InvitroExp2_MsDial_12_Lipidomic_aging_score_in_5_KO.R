### Lipidomic aging score in all KOs from In vitro Experiment #2
...
load("./Output_Data/Exp2_287_conc_lipid.MSD.format.Rdata") 
...
## -------------------------------------------------------------------------------------------------------------------
lpd.zs <- MsD.lpd.rmv.abc(E2MsD.frmt)
...
highold.mean.lpd.zs <- highold.mean.lpd.zs %>% 
  group_by(Sample) %>%
  summarise(., mean_zs = mean(zscore, na.rm = T)) %>%
  mutate(., Culture = substr(Sample, 1, str_locate(Sample, "_")-1)) %>%
  mutate(., KO = substr(Sample, str_locate(Sample, "_")+1, nchar(Sample))) %>%
  mutate(., Age = ifelse(grepl("Y", Culture), "Young", "Old")) %>% 
...
stat.test <- Cmb.avg %>%
  group_by(Age) %>%
  t_test(Cmb.avg.lpd.db.mean.zs ~ KO, paired = F, ref.group = "N") %>%
...
ggsave(filename = paste0("./Figure_Panels/Fig.4e.pdf"), width = 5, height = 5, useDingbats=FALSE)

## -------------------------------------------------------------------------------------------------------------------
pval.aging <- t.test(Cmb.avg$Cmb.avg.lpd.db.mean.zs[Cmb.avg$Age == "Young" & Cmb.avg$KO == "N"],
                     Cmb.avg$Cmb.avg.lpd.db.mean.zs[Cmb.avg$Age == "Old" & Cmb.avg$KO == "N"])
pval.aging.num <- pval.aging$p.value
## -------------------------------------------------------------------------------------------------------------------

med.sc <- Cmb.avg %>% 
  group_by(Age, KO) %>% 
  summarise(Med.sc = median(Cmb.avg.lpd.db.mean.zs))
med.sc
