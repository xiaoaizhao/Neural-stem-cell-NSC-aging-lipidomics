...
my.lipids <-  as.data.frame(t(Exp2.log2.impt.norm.conc.MsD))
...
p+geom_point(aes(shape = factor(age)), size=4, alpha=0.9)+
  theme_classic()+
  ...
  labs(title = "PCA In vitro Experiment #2")+
...
pko <- c("_N", "_M")
mtx <- Exp2.log2.impt.norm.conc.MsD %>% 
...
ggsave(filename = "./Plots/Fig.4f.pdf", width = 5, height = 5, useDingbats=FALSE)
