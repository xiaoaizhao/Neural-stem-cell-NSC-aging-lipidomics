...
all2$Exp <- factor(all2$Exp, levels = c("In vitro", "In vitro Exp #2"))
...
ggsave(paste0("./Figure_Panels/EDFig.1f.invitro.pdf"), width = 5, height = 5)

## ==== Plot in vitro Exp #2 next ====
exp2 <- all2 %>% 
  filter(Exp == "In vitro Exp #2")
...
a+
  geom_point(aes(color = Sig.lbl), alpha = 0.85, size = 3)+
  scale_y_break(
    breaks    = c(3, 5.5),          # cut from 7 to 17
...
ggsave(paste0("./Figure_Panels/EDFig.1f.invitro.Exp2.pdf"), width = 5, height = 5)
