...
a <-ggplot(c2of2all.p, aes(x = fct_reorder(Cla_SC.l, MeanES), y = `Effect size`))
a+
  ...
  stat_summary(aes(x=Cla_SC.l,y=MeanES), fun=mean, geom = "point", size=5, shape=20, alpha = 0.95, colour = "grey15") +
  theme_classic() +
...
ggsave(filename = "./Figure_Panels/Fig.2C.pdf", width = 5, height = 5, useDingbats = FALSE)
