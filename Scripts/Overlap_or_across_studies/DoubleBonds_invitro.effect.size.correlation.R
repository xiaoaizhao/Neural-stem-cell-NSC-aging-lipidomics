## Correlation analysis on double bond composition effect size with age, on all datasets on Primary NSC cultures from LC-MS/MS and Lipidyzer lipidomics
...
conc.db.pct.all3<- list(Exp3.Qui.CONC.DB.es.g, 
                        Exp2.ctrl.conc, 
                        LC.Invitro.DB.es.g) %>% 
  reduce(full_join, by = c("Cla_DB")) %>% 
  rename("ES.DB_Exp3" = "es_g.x") %>% 
  rename("ES.DB_Exp2" = "es_g.y") %>% 
  rename("ES.DB_Exp1" = "es_g")

EfSz.df.all.1x1 <- conc.db.pct.all3 %>% 
...
## ====Make plot====
pdf(file = paste0("./Figure_Panels/EDFig.4e.pdf"))
pairs(EfSz.df.all.1x1,
